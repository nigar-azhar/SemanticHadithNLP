...
def find_persons_mentioned_in_all_hadith(hadith_df, save_result=False, save_file_path=""):
    ...
    ac = []
    all_hnum = []
    with tqdm.tqdm(total=len(hadith_df), desc=f'Getting people') as pbar:

        for index, row in hadith_df.iterrows():
            # print(index)

            arabic_text = row["~Tarabic~"]#tarabic_name]
    ...
    #print(set(ac))
    result_df = pd.DataFrame({'hadith_number': all_hnum, 'person': all_crimes})
    if save_result:
