...
excel_file_path = 'dictionaries/animals.csv'
...
def find_animals_in_one_hadith(ar_text, en_text, df=df_q):
...
def find_animals_mentioned_in_all_hadith(hadith_df, save_result=False, save_file_path=""):
    ...
    with tqdm.tqdm(total=len(hadith_df), desc=f'Getting animals') as pbar:
        ...
        for index, row in hadith_df.iterrows():
            ...
            plist = find_animals_in_one_hadith(arabic_text,en_text)
    ...
    print(set(ac))
    result_df = pd.DataFrame({'hadith_number': all_hnum, 'animals': all_c})
    if save_result:
