...
excel_file_path = 'dictionaries/qo-group-of-people.xlsx'

# Read the Excel file into a DataFrame
df_q = pd.read_excel(excel_file_path)

excel_file_path_h = 'dictionaries/new-group-of-people.xlsx'

# Read the Excel file into a DataFrame
df_h = pd.read_excel(excel_file_path_h)



def find_clans_in_one_hadith(ar_text, en_text, df):
...
def find_clans_mentioned_in_all_hadith(hadith_df, save_result=False, save_file_path=""):
    ...
    ac = []
    all_hnum = []
    with tqdm.tqdm(total=len(hadith_df), desc=f'Getting clan names') as pbar:
        ...
        for index, row in hadith_df.iterrows():
            ...
            plist = find_clans_in_one_hadith(arabic_text,en_text, df_q)
            plist.extend(find_clans_in_one_hadith(arabic_text, en_text,df_h))
